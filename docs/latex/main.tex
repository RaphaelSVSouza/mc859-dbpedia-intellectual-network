...
A análise foi dividida em três partes. Primeiro, estudaram-se propriedades estruturais do grafo, como distribuição de graus e componentes fortemente conexas. Em seguida, aplicou-se PageRank ao grafo de treino em três projeções distintas: um baseline heterogêneo, uma projeção voltada a pessoas e uma projeção institucional. Por fim, o Personalized PageRank foi empregado em um experimento de predição de arestas.
...
O PageRank foi proposto originalmente para estimar a importância de páginas da Web a partir da estrutura de hiperlinks~\cite{pagerank}. A ideia central é recursiva: um vértice recebe maior pontuação quando é apontado por vértices que também possuem alta pontuação. Na convenção não normalizada utilizada pelo Neo4j GDS, para um grafo direcionado $G=(V,E)$, a recorrência pode ser expressa por:

\begin{equation}
PR^{(i+1)}(v)=(1-d)
+d\sum_{u\in Pred(v)}\frac{PR^{(i)}(u)}{\operatorname{outdeg}(u)},
\label{eq:pagerank}
\end{equation}

onde $Pred(v)$ é o conjunto de predecessores de $v$, $\operatorname{outdeg}(u)$ é o grau de saída de $u$ e $d$ é o fator de amortecimento. O termo $(1-d)$ injeta uma pontuação básica em cada vértice a cada iteração. Diferentemente da formulação probabilística que utiliza $(1-d)/|V|$, o GDS retorna por padrão escores brutos não normalizados; essa diferença de escala não altera a ordenação dos vértices~\cite{neo4j-pagerank}.

Os experimentos utilizaram a implementação do Neo4j GDS, com fator de amortecimento $d=0{,}85$ e limite de 20 iterações. Esses valores também correspondem aos padrões documentados pela biblioteca~\cite{neo4j-pagerank}. As arestas foram tratadas como não ponderadas e nenhum escalonador foi configurado. Como a análise utiliza a ordem relativa, a ausência de normalização não altera os rankings.
...
função PageRank(Grafo G, real d, inteiro maxIterações):
    ...
    para iteração <- 1 até maxIterações:
        ...
        para cada vértice u de G:
            ...
            senão:
                u não produz contribuição de saída nesta iteração
...
Na execução efetiva, a iteração e o tratamento de vértices sem saída são realizados internamente por \texttt{gds.pageRank}. O script \texttt{neo4j/pagerank.py} é responsável por criar a projeção apropriada, executar o algoritmo, gravar os escores na propriedade \texttt{pageRank} e exportar o ranking para TSV. O pseudocódigo explicita a recorrência dos escores brutos adotada pelo GDS, sem aplicar normalização posterior.

\subsection{Projeções avaliadas}

No fluxo integrado dos experimentos, o conjunto de teste é selecionado e removido antes da execução dos três cenários de PageRank, conforme detalhado na Subseção~\ref{subsec:split}. Assim, as projeções são construídas sobre $G_{treino}=G\setminus E_{teste}$, que preserva todos os vértices e contém as relações não ocultadas. Neste relatório, \emph{baseline} designa a projeção heterogênea com todos os nove tipos de relacionamento disponíveis em $G_{treino}$, e não o grafo original antes da separação entre treino e teste.

O primeiro experimento executou o PageRank sobre esse baseline, preservando a orientação natural das relações. Esperava-se encontrar, nas primeiras posições, figuras semelhantes às observadas no núcleo exploratório da Figura~\ref{fig:nucleo}, como Immanuel Kant, Leo Strauss e Karl Marx. O ranking obtido, entretanto, foi bastante diferente: além de instituições, religiões e áreas do conhecimento, sua primeira posição geral foi ocupada por Vaklush Tolev, figura búlgara associada à religião e à teologia~\cite{vaklush-dbpedia}, com escore 488,2684.
...
\textbf{Baseline.} A projeção inclui os nove tipos finais de relacionamento presentes no grafo de treino, em suas orientações naturais. Esse experimento mede centralidade global no grafo heterogêneo remanescente e serve como referência para as demais configurações.
...
Para uma aresta oculta $(s,r,t)$, o ranking produzido a partir de $(s,r)$ é interpretado como uma lista de destinos candidatos. A predição é considerada recuperada quando $t$ aparece nessa lista. A avaliação é não filtrada: o script exclui a própria origem, mas não remove da lista os destinos que já possuem uma ligação observada do tipo $r$ com $s$ em $G_{treino}$. Essas ligações conhecidas podem ocupar posições no Top-$K$, tornando a avaliação mais conservadora, mas não provocam vazamento da aresta de teste, que foi removida antes da projeção. O procedimento completo é sintetizado no pseudocódigo a seguir.
...
Desde a concepção do projeto, o objetivo central foi investigar se relações extraídas da Wikipédia, uma fonte ampla e colaborativa, seriam suficientes para recuperar padrões compatíveis com referências externas mais curadas. Os rankings não receberam informações da SEP ou do CWUR durante seu cálculo: eles foram produzidos exclusivamente pela topologia da DBpedia e somente depois confrontados com essas referências. Para pessoas, construiu-se a partir da SEP uma ordenação por presença em entradas de uma enciclopédia acadêmica; para universidades, utilizou-se o Top 20 do CWUR. O cruzamento permitiu avaliar se o reconhecimento registrado por fontes independentes também emergia da estrutura das relações da Wikipédia. Essas referências não foram tratadas como verdades absolutas, pois também possuem critérios e vieses próprios, mas como parâmetros externos mais controlados do que o conjunto heterogêneo usado para construir o grafo.

Os resultados mostraram uma concordância parcial e sensível à modelagem. A projeção invertida de pessoas elevou a correlação de Spearman com a SEP de 0,230920 para 0,499841 e o Overlap@50 de 4\% para 40\%, indicando que a rede contém sinal compatível com o reconhecimento externo quando o fluxo é orientado de acordo com a interpretação de influência. Já a projeção institucional não superou o baseline, demonstrando que restringir relações não garante melhoria automática. O caso de Vaklush Tolev evidenciou como hubs religiosos e direções inadequadas podem criar funis de prestígio que não correspondem à expectativa histórica.

Na predição de ligações, o PPR recuperou 30,85\% das arestas ocultadas no Top-500, mas obteve HitRate@20 de apenas 4,22\% e MRR de 0,011418. Quase todos os acertos vieram de \texttt{INFLUENCED}, cuja topologia oferece caminhos alternativos; relações bipartidas, como \texttt{ALMAMATER} e \texttt{RELIGION}, tornaram seus destinos inalcançáveis após a ocultação da ligação direta. Conclui-se que dados derivados da Wikipédia podem revelar padrões de influência corroborados parcialmente por referências curadas, mas proximidade estrutural, notoriedade histórica e proximidade semântica não são equivalentes. Trabalhos futuros podem combinar topologia e informação textual, investigar projeções heterogêneas ponderadas e empregar validações temporais, sempre preservando um protocolo sem vazamento de dados.
